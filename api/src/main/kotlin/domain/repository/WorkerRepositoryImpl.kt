package com.sayesamanappointmentbookingsystem.domain.repository

import com.sayesamanappointmentbookingsystem.database.model.WorkerModel
import com.sayesamanappointmentbookingsystem.data.repository.WorkerRepository

class WorkerRepositoryImpl: WorkerRepository {
    override fun add(item: WorkerModel): Boolean {
        TODO("Not yet implemented")
    }

    override fun edit(item: WorkerModel): Boolean {
        TODO("Not yet implemented")
    }

    override fun delete(item: WorkerModel): Boolean {
        TODO("Not yet implemented")
    }

    override fun all(): List<WorkerModel> {
        TODO("Not yet implemented")
    }

    override fun getBy(id: Int?, sname: String?, bookingdailylimit: Int?): WorkerModel {
        TODO("Not yet implemented")
    }

    override fun login(user: String, pass: String): Boolean {
        TODO("Not yet implemented")
    }

    override fun changePass(user: String, pass: String, againPass: String): Boolean {
        TODO("Not yet implemented")
    }


}